...
class FlickrTrainDataset(Dataset):
    ...
    def __init__(self, csv_path: str, img_dir: str, split: str, transform, skip_missing=False):
        assert split == "train", "FlickrTrainDataset must use split='train'."
        ...
        print("Each image randomly samples one caption.")
        print("=" * 80)
...
def build_eval_data(
    ...
    训练脚本里只会传 split='val'。
    """
    assert split in ["val", "test"], "Evaluation split must be 'val' or 'test'."
...
class LoRAMultiheadAttention(nn.Module):
    """
    给 PyTorch nn.MultiheadAttention 加 LoRA。

    对 q / v / out 加 LoRA：
    q = Wq x + LoRA_q(x)
    v = Wv x + LoRA_v(x)
    out = Wo attn + LoRA_out(attn)

    OpenCLIP 里的 attention 通常就是 nn.MultiheadAttention。
    """
    def __init__(
        self,
        ...
        assert base_attn._qkv_same_embed_dim, "This LoRA wrapper only supports qkv_same_embed_dim=True."
    ...
    def forward(
        ...
        """
        兼容 nn.MultiheadAttention 的常见调用方式。

        OpenCLIP 通常传入：
        query/key/value: [seq_len, batch, dim]
        need_weights=False
        attn_mask: text transformer 里可能有 causal mask
        """
...
def inject_lora_to_attention(
    ...
    """
    递归替换所有 nn.MultiheadAttention 为 LoRAMultiheadAttention。
    """
    num_replaced = 0
...
def save_checkpoint(
    ...
    print(f"Saved best LoRA checkpoint to: {save_path}")
...
def main():
    args = types.SimpleNamespace(
        ...
        best_ckpt_name="best_lora_clip.pt",
    ...
    print("=" * 80)
    print("CLIP LoRA Finetune Training")
    print("=" * 80)
    ...
    if num_replaced == 0:
        raise RuntimeError(
            "No nn.MultiheadAttention modules were found. "
            "Please inspect OpenCLIP module names for this model."
        )

    print(f"Injected LoRA into {num_replaced} MultiheadAttention modules.")

    # ===== 关键修复：将新加入的 LoRA 层也移动到指定设备 =====
    model = model.to(args.device)   # 确保所有参数（包括新创建的 LoRA）都在同一设备
    # ========================================================
